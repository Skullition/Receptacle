...
public class WaterBalloonItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            WaterBalloonEntity waterBalloonEntity = new WaterBalloonEntity(world, user);
            waterBalloonEntity.setItem(itemStack);
            waterBalloonEntity.setProperties(user, user.getPitch(), user.getYaw(), 0.0F, 1.5F, 1.0F);
            world.spawnEntity(waterBalloonEntity);
        }
